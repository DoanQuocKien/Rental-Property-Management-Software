  ...
  db.all(`PRAGMA table_info(${tableName})`, (err, columns) => {
    ...
    if (!exists) {
      ...
      db.run(`ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${safeColumnDef}`, (alterErr) => {
        if (alterErr) console.error(`Failed to add column ${tableName}.${columnName}:`, alterErr.message);
      });
    }
  });
}
...
// ── Schema init ───────────────────────────────────────────────
db.serialize(() => {
  // ★ WAL mode: cho phép đọc đồng thời, tránh SQLITE_BUSY khi có nhiều request
  db.run('PRAGMA journal_mode = WAL');

  // ★ Tăng timeout chờ lock lên 10 giây (tránh "database is locked" error)
  db.run('PRAGMA busy_timeout = 10000');

  // ★ Tắt synchronous full để tăng tốc ghi (vẫn an toàn với WAL)
  ...
  // ★ Dọn dẹp token hết hạn mỗi giờ (tránh DB phình to)
  setInterval(() => {
    db.run(
